...
function salvarAlunoEMatricula(a,m) { salvarAluno(a); const sh=SpreadsheetApp.getActive().getSheetByName(ABA_MATRICULAS); m.idAluno=a.idAluno; m.idMatricula=m.idMatricula||('MAT-'+Date.now()); const lin=localizar(sh,'ID_MATRICULA',m.idMatricula); escreverPorHeaders(sh,matriculaObj(m),lin); return json({sucesso:true,idAluno:a.idAluno,idMatricula:m.idMatricula,versao:APP_VERSION}); }
