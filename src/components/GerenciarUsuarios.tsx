...
export const GerenciarUsuarios: React.FC = () => {
  ...
  const salvar = async (e: React.FormEvent) => { e.preventDefault(); const r=await cadastrarUsuario(nome,login,senha); setMsg(r.mensagem); if(r.sucesso){setNome('');setLogin('');setSenha('');await recarregar();} };
  return <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
    ...
    <form onSubmit={salvar} className="grid md:grid-cols-4 gap-3 bg-slate-50 border rounded-xl p-4">
