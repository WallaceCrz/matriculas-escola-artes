...
export const Header: React.FC<HeaderProps> = ({ etapaAtual,onAbrirModalConfig,appsScriptConectado,modoVisualizacao,setModoVisualizacao,sessao,onSair }) => {
 ...
 return <header className="bg-indigo-900 text-white shadow-md border-b border-indigo-950 sticky top-0 z-30">
  <div className="max-w-7xl mx-auto px-4 py-3 flex flex-wrap items-center justify-between gap-3">
   <button type="button" onClick={()=>setModoVisualizacao('inicio')} className="flex items-center gap-3 text-left rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-300" title="Voltar ao início"><div className="w-10 h-10 rounded-xl bg-amber-400 text-indigo-950 flex items-center justify-center"><Drama className="w-6 h-6"/></div><div><h1 className="font-extrabold text-lg uppercase">Escola de Artes</h1><div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-indigo-200"><span className="inline-flex items-center gap-1"><UserCircle2 className="w-3.5 h-3.5"/>{sessao.nome}</span><span className="inline-flex items-center gap-1 font-mono" title="Tempo restante da sessão"><Clock3 className="w-3.5 h-3.5"/>{formatarTempoRestante(tempoRestante)}</span></div></div></button>
